...
let universalABI = JSON.parse(
  fs.readFileSync("UNISWAP_UNIVERSAL_ABI.json", "utf-8")
);
...
function buildTransactionObject(transactionDetails, decodedFunction) {
  let methodName;
  if (
    v2VersionDictionary["swapExactETHForTokens"].includes(
      decodedFunction.function
    )
  ) {
    methodName = "swapExactETHForTokens";
  } else if (
    v2VersionDictionary["swapETHForExactTokens"].includes(
      decodedFunction.function
    )
  ) {
  ...
  return {
    ...
    estimatedBlocksUntilConfirmed:
      transactionDetails.estimatedBlocksUntilConfirmed,
...
function decodeExecute(transactionInput) {
  const parsedTx = universalInteface.parseTransaction({
